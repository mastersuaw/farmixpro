...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
REST API de FarmixPro para empresas, productos/variantes, facturación, impuestos y pagos.

## Autenticación

Usa **Laravel Sanctum** (Bearer token). Tras `POST /auth/login` o `POST /auth/register`, envía:

`Authorization: Bearer {token}`

## Multi-empresa

La empresa actual se resuelve en este orden:

1. Cabecera `X-Company-Id` (debe ser una empresa del usuario)
2. Empresa seleccionada con `POST /companies/{id}/switch`
3. Primera empresa vinculada al usuario

Las rutas de negocio responden `422` si el usuario no tiene empresa.

## Respuestas

Todas las respuestas de `/api/*` usan el envelope `{ success, message, data, errors }`. Los listados paginados incluyen `meta`.
MD,
    ],
